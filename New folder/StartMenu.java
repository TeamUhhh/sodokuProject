...
public class StartMenu extends JFrame implements ActionListener{
	...
	public StartMenu(Boolean defaultInput, Boolean checkDropDown, Boolean checkButtonSelect){
		...
		// decides what button in settings the user has pressed
	...
	public void actionPerformed(ActionEvent e) {
		...
		// decides which layout the user selected then creates a new "Easy" GridLayout
		if(getAction.equals("Easy")){
			if(dropDownCheck){
				GridDropDown nineGrid = new GridDropDown(9, 9, "Easy", true, 1); // checkDemovar to check if "Demo" has been selected
				...
				GridLayoutPage nineGrid = new GridLayoutPage(9, 9, "Easy", false, false, null, false, null); // checkDemovar to check if "Demo" has been selected
				nineGrid.setVisible(true);
			}
		}
		// decides which layout the user selected then creates a new "Hard" GridLayout
		else if(getAction.equals("Hard")){
			if(dropDownCheck){
				GridDropDown nineGrid = new GridDropDown(9, 9, "Hard", true, 1); // checkDemovar to check if "Demo" has been selected
				nineGrid.setVisible(true);
			}
			else if(buttonSelectCheck){
				System.out.println("DOES NOTHING");
				...
				GridLayoutPage nineGrid = new GridLayoutPage(9, 9, "Hard", false, false, null, false, null); // checkDemovar to check if "Demo" has been selected
				nineGrid.setVisible(true);
			}
		}
		// creates a new Settings Page
		else if(getAction.equals("Settings")){
			SettingsLayoutPage settings = new SettingsLayoutPage(defaultSelect, dropDownCheck, buttonSelectCheck); 
